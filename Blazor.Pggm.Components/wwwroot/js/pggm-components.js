...
class EventListenerManager {
  ...
  addListener(element, eventName, dotNetRef, methodName, useTimeout = true) {
    ...
    if (this.listeners.has(key)) {
      console.warn(`EventListenerManager: Listener already exists for ${eventName} on element`);
      return;
    }
  ...
  removeListener(element, eventName) {
    ...
    if (listenerInfo) {
      listenerInfo.element.removeEventListener(listenerInfo.eventName, listenerInfo.handler);
...
(function() {
  ...
  window.PggmComponents.addEventListener = function(element, eventName, dotNetRef, methodName, useTimeout = true) {
  ...
  window.PggmComponents.addAccordionItemToggleListener = function(element, dotNetRef) {
    if (element) {
      ...
      eventManager.listeners.set(eventManager._getKey(element, 'accordionItemToggle'), {
        element,
  ...
  window.PggmComponents.setupDialogEventHandlers = function(element, dotNetRef) {
    if (element) {
      ...
      eventManager.listeners.set(eventManager._getKey(element, 'cancelDialog'), {
        element,
...
class PggmDesignSystem {
  ...
  async _loadBundle() {
    ...
    try {
      ...
      script.src = "./_content/Blazor.Pggm.Components/js/bundle.js";
      script.id = "pggm-bundle";
      script.type = "module";
      
      return new Promise((resolve, reject) => {
        script.onload = () => resolve();
        script.onerror = () => reject(new Error('Failed to load PGGM bundle'));
  ...
  _loadPElementsCore() {
    ...
    const script = document.createElement("script");
    script.src = "https://cdn.jsdelivr.net/npm/p-elements-core@1.2.31/dist/p-elements-core-modern.min.js";
