...
public partial class PggmInputIban : PggmEventComponentInputBase<string>
{
    public override string TagName => "pggm-input-iban";

    private string? _lastSyncedValue;
    private readonly string _dataId = $"pggm-input-iban-{Guid.NewGuid()}";
    ...
    /// <summary>
    /// Whether to auto-format the IBAN input
    /// </summary>
    ...
    /// <summary>
    /// Event callback for IBAN validation result
    /// </summary>
    [Parameter] public EventCallback<bool> OnValidationChanged { get; set; }

    /// <summary>
    /// Event callback for invalid IBAN input
    /// </summary>
    [Parameter] public EventCallback<EventArgs> OnInvalid { get; set; }

    /// <summary>
    /// Error message added to the EditContext when the web component's IBAN validation
    /// fails. Only used inside an EditForm. Defaults to "IBAN is ongeldig".
    /// </summary>
    [Parameter] public string? IbanValidationMessage { get; set; }
    ...
    private async Task ValidateIbanIntoStore()
    {
        if (_messageStore == null || EditContext == null) return;

        _messageStore.Clear(FieldIdentifier);

        var hasCustomError = await GetIbanCustomErrorAsync();

        if (hasCustomError)
        {
            var msg = _resolvedValidationMessage ?? IbanValidationMessage ?? "IBAN is ongeldig";
            _messageStore.Add(FieldIdentifier, msg);
        }
    }

    private async Task<bool> GetIbanCustomErrorAsync()
    {
        try
        {
